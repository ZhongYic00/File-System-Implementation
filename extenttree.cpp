...
void ExtentTree::pushup(node* p)
{
    ...
    return;
        ...
        p->flag = 0;
        for (int i = 0; i < log(p->bl) / log(2); i++)p->flag |= 1 << i;
        return;
    }
...
int ExtentTree::update(node* p, int l, int r, int ref,LBA_t blk,LBA_t target)
{
    ...
    {
        p->left = 0; p->ref = ref; p->maxleft = 0;
        p->flag = 0; NumOfUpd++;
        return p->l;
    }
    int mid = (l + r) / 2;
    if (p->lson == nullptr)//build new node on tree 
    {
        ...
        p->maxleft = blk;
        for (int i = 0; i < log(blk) / log(2); i++)p->flag |= 1 << i;
    }
...
void ExtentTree::Release(node* p, int now, LBA_t d)
{
    ...
    int mid = (p->l + p->r) / 2;
    if (now > mid)Release(p->rson, now, d);
    else Release(p->lson, now, d);
    pushup(p);
}
void ExtentTree::setRoot()
{
    if (setroot)return;
    for (int i = 0; i <= 30; i++)root->flag |= 1<<i;
    root->l = 1; root->r = SumBlo;
    root->bl = SumBlo; root->left = SumBlo; root->ref = 0;
    setroot = true; 
}
void ExtentTree::build(node* p, int l, int r, int ref, LBA_t blk, LBA_t target, int pos)
{
    ...
    if (pos <= mid)build(p->lson, l,mid ,ref, blk >> 1, target, pos);
    if (p->rson == nullptr)//build new node on tree 
    {
        p->rson->bl = blk, p->rson->l = mid + 1, p->left = blk;
        p->maxleft = blk;
    ...
    if (pos > mid)build(p->rson, mid + 1, r,ref, blk >> 1, target, pos);
    pushup(p);
}
...
list<pair<LBA_t, LBA_t>> ExtentTree::allocateExtents(LBA_t blks)
{
    ...
    while (now)
    {
        if ((root->flag >> dat) & 1) { X.push_back(make_pair( update(root, 1, SumBlo, 1, SumBlo, 1 << dat),dat)); now -= 1 << dat; dat--; continue; }
        dat--;
    }
    return X;
}
